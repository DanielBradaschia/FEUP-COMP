...
public class CodeParser {
    ...
    private int temp = 0;
    private int stack = 0;
    private SimpleNode program;

    public CodeParser(SimpleNode root) {
        this.program = root;
        this.root = root.getChild(0);
        this.builder = new StringBuilder();
        SimpleNode n = program.next();
        do {
            if (n.getName().equals("classDeclaration"))
                break;
        } while ((n = program.next()) != null);
        this.program.reset();
        this.classe = n.next().getName();
    ...
    private void generateHeader() {
        ...
        this.root = this.program.next();

        while (this.root != null && this.root.getName().equals("Import")) {
            this.root = this.program.next();
        }
        write(".class public ");
        write(this.classe);
        ...
        SimpleNode extend = this.root.next(1);

        if (extend != null && extend.getName().equals("extends")) {
            write(".super ");
    ...
    private void generateMethods() {
        ...
        while ((method = this.root.next()) != null) {
            if (!method.getName().equals("public"))
                continue;
            nl();
            method = method.next();
    ...
    private void attribution(SimpleNode node) {
        if (!node.next().getName().equals("array")) {
            ...
            if (globals.contains(node.same().getName())) {
                ...
                tab();
                write(getType2(
                        JmmParser.getInstance().getMethod(this.method).getSymbolType(node.previous().getName())));
                if (idx > 3)
                    write("store ");
                else
                    write("store_");
    ...
    private void functionCall(SimpleNode caller, SimpleNode call) {
        ...
        if(method == null && parameters != null){
            for (String name : JmmParser.getInstance().getMethods().keySet()) {
                String[] tokens = name.split(",");
                if (name.contains(call.same().getName()) && tokens.length == parameters.children.length) {
                    method = JmmParser.getInstance().getMethod(name);
                    break;
                }
    ...
    private void returns(Table method, SimpleNode call, boolean pop) {
        ...
        if (method != null) {
            ...
            if (!type.equals("V"))
                inc();
            write(type);
            if (!parentName.equals("!") && !parentName.equals("parameters") && !parentName.equals("return")
                    && !parentName.equals("condition") && pop && !type.equals("V")
                    && findReturnType(call).equals("void")) {
                nl();
                tab();
                dec();
                write("pop");
    ...
    private String findReturnType(SimpleNode call) {
        ...
        if (parentName.equals("+") || parentName.equals("-") || parentName.equals("*") || parentName.equals("/")
            ...
            if (((SimpleNode) parentElement.children[0]).getName().equals("array"))
                ...
                return JmmParser.getInstance().getMethod(this.method)
                        .getSymbol(((SimpleNode) parentElement.children[0]).getName()).getType();
        }
        return "void";
    }
